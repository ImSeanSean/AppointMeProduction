...
export class AppointmentValidationService {
  ...
  public getTeacherDayAppointments(teacherId: string, date: Date): Observable<Appointment[]> {
    return this.http.get<Appointment[]>(`http://localhost/appointme/pdo/api/get_day_appointments/${teacherId}/${date}`)
  }
  public getDaySchedule(teacherId:string, day: number){
    return this.http.get<DaySchedule[]>(`http://localhost/appointme/pdo/api/get_day_schedule_student/${teacherId}/${day}`)
  }
